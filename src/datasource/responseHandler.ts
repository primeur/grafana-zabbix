...
function convertHistory(history, items, addHostName, convertPointCallback) {
    ...
    return _.map(grouped_history, (hist, itemid) => {
        ...
        console.log('Zabbix item:', item.name, 'tags:', item.tags);

        // Add item tags as scopedVars
        if (item.tags && Array.isArray(item.tags)) {
            for (const t of item.tags) {
                scopedVars[`__zbx_item_tag_${t.tag}`] = { text: '', value: t.value };
            }
            console.log('Zabbix item tags scopedVars:', scopedVars);
        }
